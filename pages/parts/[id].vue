...
<template>
  <div class="min-h-screen bg-surface pb-32 lg:pb-16">
    <!-- Breadcrumb -->
    ...
    <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <div class="grid lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-5 space-y-4">
          ...
          <!-- Thumbnails -->
          <div v-if="part.images.length > 1" class="flex gap-3 overflow-x-auto no-scrollbar pb-1">
            <button
              v-for="(img, idx) in part.images"
              :key="idx"
              :class="[
                'w-20 h-20 rounded-xl overflow-hidden border-2 transition-all shrink-0 touch-manipulation',
            ...
            <div>
              ...
              <div class="border border-border rounded-2xl overflow-hidden divide-y divide-border text-xs">
                <div
                  ...
                  :key="key"
                  class="flex flex-col sm:flex-row justify-between p-3 bg-surface gap-1"
                >
                  <span class="text-muted font-medium">{{ key }}</span>
                  <span class="text-gray-900 font-semibold text-left sm:text-right">{{ val }}</span>
                </div>
              </div>
            </div>
      ...
      <!-- Sticky Mobile Quick Purchase CTA Bar -->
      <div class="lg:hidden fixed bottom-14 left-0 right-0 z-30 bg-white/95 backdrop-blur-md border-t border-border px-4 py-2.5 shadow-lg flex items-center justify-between gap-3">
        <div class="min-w-0 flex-1">
          <div class="font-mono font-extrabold text-primary text-base truncate">
            {{ formatLKRFull(part.price) }}
          </div>
          <div class="text-[10px] font-semibold" :class="part.inStock ? 'text-emerald-600' : 'text-primary'">
            ● {{ part.inStock ? 'In Stock' : 'Out of Stock' }} · {{ part.brand }}
          </div>
        </div>
        ...
        <div class="flex items-center gap-2 shrink-0">
          <a
            ...
            target="_blank"
            rel="noopener"
            class="flex items-center justify-center gap-1.5 px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs shadow-xs min-h-[42px] touch-manipulation"
          >
          ...
          <a
            :href="`tel:${part.seller.phone}`"
            class="flex items-center justify-center px-3.5 py-2 bg-gray-900 hover:bg-primary text-white font-bold rounded-xl text-xs shadow-xs min-h-[42px] touch-manipulation"
          >
            📞 Call
          </a>
        </div>
