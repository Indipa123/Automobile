...
<template>
  <div class="min-h-screen bg-background">
    ...
    <template v-else-if="car">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 pb-28 lg:pb-6">
        <!-- Breadcrumb -->
            ...
            <div class="border-b border-border">
              <div class="flex gap-1 overflow-x-auto no-scrollbar">
                <button
                  ...
                  :key="tab.id"
                  :class="[
                    'px-4 sm:px-5 py-3 text-xs sm:text-sm font-medium border-b-2 transition-colors whitespace-nowrap',
                    activeTab === tab.id
        ...
        <div class="lg:hidden fixed bottom-14 left-0 right-0 z-30 bg-white/95 backdrop-blur-md border-t border-border px-4 py-2.5 shadow-lg flex items-center justify-between gap-3">
          <div class="min-w-0 flex-1">
            <div class="flex items-baseline gap-1.5">
              <span class="font-mono font-extrabold text-primary text-base truncate">{{ formatLKRFull(car.price) }}</span>
            </div>
            <p v-if="car.negotiable" class="text-[10px] text-muted leading-none">Negotiable · {{ car.location.city }}</p>
            <p v-else class="text-[10px] text-muted leading-none">{{ car.location.city }}</p>
          </div>
          ...
          <div class="flex items-center gap-2 shrink-0">
            <a
              ...
              target="_blank"
              rel="noopener"
              class="flex items-center justify-center gap-1.5 px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl text-xs shadow-xs min-h-[42px] touch-manipulation"
            >
            ...
            <a
              ...
              class="flex items-center justify-center px-3.5 py-2 bg-primary hover:bg-primary-dark text-white font-semibold rounded-xl text-xs shadow-xs min-h-[42px] touch-manipulation"
            >
              📞 Call
            </a>
          </div>
